...
```{r relevant functions, echo=FALSE}
# The Likelihood
...
for (i in 1:it) {
  ...
  # proposal <- matrix(c(min(rnorm(1, chain[i,1], val1),10), min(rnorm(1, chain[i,2], val2),10), rbinom(1, n, chain[i,3]/n)), nrow =1, ncol=3)
...
theta_0 <- c(0, 4, 10)
chain[1, ] <- theta_0
  proposal <- rnorm(3, mean = chain[1, ], 
                    sd = c(0.5,0.1,0.3))
  p_move <- exp(posterior(proposal) - posterior(chain[1, ]))
```
...
joint_post <- function(mu, lambda, m, data){
  joint_density <- posterior(mu) + posterior(lambda) + posterior(m)
