...
$$f_n\left(\bar{x} \mid p \right) = {n \choose x} p ^ {x} \left( 1 - p \right) ^ {n - x}$$
...
$$\frac{\partial {L\left( p \right)}}{\partial p} = \frac{ \partial \left({log {n \choose x} + x log(p) + (n-x)log(1-p)}\right) } {\partial p} = \frac{x}{p} - \frac{n-x}{1-p}$$
...
We set these, respectively, equal to the sample moments $m_1$ and $m_2$, and then use relations (1) and (2) to solve for $\alpha$ and $\beta$. 
